...
async def logger_id(client):
    desc = "A Bot Logger Group For Tigeruserbot. DO NOT LEAVE THIS GROUP!!"
    ...
    try:
        grp = await client(functions.channels.CreateChannelRequest(title="Tigeruserbot Logger", about=desc, megagroup=True))
...
<b><i>»» <u><a href='https://t.me/TigerNetwork'>†hê TigerUserBot</a></u> ««</i></b>
"""
    await client.send_file(
        ...
        caption=text,
        parse_mode="HTML",
        buttons=[[Button.url("Code Tiger", "https://t.me/CodeTiger")]],
    )
...
async def join_it(client):
    if client:
        try:
            await client(functions.channels.JoinChannelRequest("@TigerNetwork"))
            await client(functions.messages.ImportChatInviteRequest('+6G6S9h-5YYgzMzBl'))
